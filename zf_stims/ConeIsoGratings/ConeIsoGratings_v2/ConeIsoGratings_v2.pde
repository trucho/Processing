...
/*
  ...
  Oct 2025 (Angueyra):
    Current setup has up and down color that get modulated from 0 to 1 in counterphase and counter directions
    It probably makes more sense to start from a base color and do deviations in the M and L direction, no?  
*/
...
float contrast = 0.10; // Test grating's weber contrast as fraction between 0 and 1 


int[] testColor = {128,0,0}; 
int[] refColor = {0,34,0};

//// L isolating (Oct 27, 2025)
//int[] upColor = {255,0,11}; 
//int[] downColor = {0,1,13};

//// M isolating (Oct 27, 2025)
//int[] upColor = {0,58,0}; 
//int[] downColor = {255,2,0};

//// S isolating (Oct 27, 2025)
//int[] upColor = {0,255,80}; 
//int[] downColor = {255,190,162};




float barWidth = 30; //30; //Oct2025: needs spatial calibration; 30 ~ default for OKR bars
float barSpeed = 4.5;//5 //Oct2025: needs spatial calibration; 5 ~ default for OKR bars
...
void draw() {
  ...
  for (int i = 0; i < width; i++) {
    for (int j = 0; j < height; j++) {
      float valMin = cos(x - phaseDrift); // cos(x) c [-1 1]
      float valMinNorm = nowTestContrast*((valMin + 1.0)/2.0); // Scale to between 0 and 1
      float upR = valMinNorm * testColor[0];
      float upG = valMinNorm * testColor[1];
      float upB = valMinNorm * testColor[2];
      float valMax = cos(x+PI + phaseDrift); // cos(x+PI) c [-1 1]
      float valMaxNorm = nowRefContrast*((valMax + 1.0)/2.0); // Scale to between 0 and 1
      float downR = valMaxNorm * refColor[0];
      float downG = valMaxNorm * refColor[1];
      float downB = valMaxNorm * refColor[2];
      
      //print(valMaxNorm);
      
      color ConeIso = color(upR+downR,upG+downG,upB+downB);
